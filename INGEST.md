...
`llms` has exactly one runtime dependency (`aiohttp`) and the gemini extension adds exactly one
(`google-genai`). That's clearly deliberate, so ingest shouldn't quietly arrive with
