...
def to_custom_metadata(meta):
    ...
    for k, v in meta.items():
        if isinstance(v, bool):
            out.append(types.CustomMetadata(key=k, string_value=str(v).lower()))
        elif isinstance(v, (int, float)):
            out.append(types.CustomMetadata(key=k, numeric_value=v))
        elif isinstance(v, list):
            out.append(types.CustomMetadata(key=k, string_list_value=types.StringList(values=v)))
        else:
            out.append(types.CustomMetadata(key=k, string_value=str(v)))
    return out
