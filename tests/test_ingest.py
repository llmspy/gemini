...
class TestCustomMetadataWireFormat(unittest.TestCase):
    """
    The payload the worker sends must satisfy the SDK's own validation.
    ...
    isolation and was rejected by pydantic before the request was ever sent - caught only by
    calling the live API, so it is pinned here.
    """
    ...
    def test_payload_passes_sdk_validation(self):
        from google.genai import types
        types.UploadToFileSearchStoreConfig(
            display_name="x", custom_metadata=self.db.to_custom_metadata(self.doc())
        )

    def test_bare_list_would_be_rejected(self):
        import pydantic
        from google.genai import types
        with self.assertRaises(pydantic.ValidationError):
            types.UploadToFileSearchStoreConfig(
                display_name="x", custom_metadata=[{"key": "versions", "string_list_value": ["v7"]}]
            )
